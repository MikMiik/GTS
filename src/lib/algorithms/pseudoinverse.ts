...
export function runPseudoinverse(params: Record<string, string>, logger: Logger): void {
  let A: Mat;
  try {
  ...
  const m = A.length, n = A[0].length;
  logger.section(`Ma Trận Nghịch Đảo Suy Rộng $A^\\dagger$ của ma trận $${m} \\times ${n}$`);
  ...
  for (let i = 0; i < rank; i++) {
    const vi = VCols[i];
    const ui = scale(A.map((row) => dot(row, vi)), 1 / sigmas[i]);
    UCols.push(ui);
  }
  if (m > rank) {
  ...
  if (maxErr < 1e-6) {
    logger.success(`Kiểm tra $A A^\\dagger A \\approx A$: ✓ Sai số tối đa ${fmtNum(maxErr)}`);
  } else {
    logger.warn(`Kiểm tra $A A^\\dagger A$: sai số = ${fmtNum(maxErr)} (có thể do độ chính xác số thực).`);
  }
}
